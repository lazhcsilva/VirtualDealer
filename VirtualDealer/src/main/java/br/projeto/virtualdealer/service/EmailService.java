 ...
 * 
 * import br.projeto.virtualdealer.dao.EmailDAO; import
 * br.projeto.virtualdealer.model.Cliente; import
 * br.projeto.virtualdealer.model.Email; import
 * br.projeto.virtualdealer.model.EmailMensagem;
 * 
 ...
 * 
 * @Autowired private EmailDAO emailRep;
 * 
 ...
 * 
 * LocalDate agora = LocalDate.now(); LocalDate validade = agora.plusDays(5);
 * 
 * 
 * EmailMensagem mensagem = new EmailMensagem(); mensagem.setTitulo("Olá, " +
 * cliente.getNome() + ". Ative sua conta!"); mensagem.
 * setMensagem("Para ter acesso total ao nosso site ative sua conta, copiando esse endereço abaixo e colando na url. (este email vence em: "
 ...
 * 
 * public void sendEmailTSL(Email email, boolean comTolken) throws
 * MessagingException {
 * 
 ...
 * InternetAddress(email.getEmailRemetente()));
 * message.setRecipients(Message.RecipientType.TO,
 * InternetAddress.parse(email.getEmailDestinatario()));
 * message.setSubject(email.getAssunto());
 * message.setContent(email.getMensagem().gerarMensagemComTolken(comTolken),
 * "text/html");
 * 
 * Transport.send(message); this.emailRep.save(email); }
 * 
 * public void enviarRecuperacaoDeSenha(String emailCliente, String nome, String
 * novasenha) throws MessagingException{
 * 
 * EmailMensagem mensagem = new EmailMensagem(); mensagem.setTitulo("Olá, " +
 * nome + "  recupere sua senha!"); mensagem.setMensagem("Sua nova senha é:  " +
 * novasenha + "  (Não se preocupe,você pode muda-la em nossa página')");
 * 
 * 
